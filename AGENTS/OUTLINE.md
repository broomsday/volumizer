...
- `volumizer/cli.py`: CLI parser/orchestration (`analyze`, `cluster`, `cache` subcommands, checkpoint/resume, progress logging (JSONL + periodic ETA), metadata cache controls, manifest write/replay, summary subset replay, deterministic cluster sharding)
